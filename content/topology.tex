...
\begin{definition}{Dicht, seperabel}
  ...
  \begin{enumerate}[label = (\arabic*)]
    ...
    \item $(X,d)$ \textbf{seperabel} $\Leftrightarrow \exists A \subset X$ 
      \textit{abz.} und \textit{dicht} in \(X\).
  \end{enumerate}
\end{definition}
...
\begin{lemma}
  \(A\subset M\) ist \textit{offen in} \(M\), gdw. es \(A'\subset X\) 
  offen gibt, mit \(A = A'\cap X\)
\end{lemma}
...
\begin{satz}{Seperabler Teilraum}
  Sei \(X\) seperabel \(\implies M\) \textit{seperabel}.
\end{satz}
